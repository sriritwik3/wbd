...
function ModifyPost({
  ...
  const [newpost, setNewpost] = useState({
    //hook to store post data and update
    title: '',
    goal: '',
    description: '',
    membercount: 1,
  ...
  useEffect(() => {
    setNewpost({
      //this hook is called  upon redering this page first time and the values are stored in the variable created above
      title: title,
      goal: goal,
  ...
  const handleinput = (e) => {
    //updates the values of the post
    let name, value;
    name = e.target.name;
  ...
  function taghandle(e) {
    //updates the tags(skills required) of the post
  ...
  async function handlesubmit(e) {
    //function updates the data in firebase
    e.preventDefault();
    try {
      ...
      {sameUser ? (
        <div className={styles.div}>
          ...
          {failure ? (
            ...
            >
              There was an Error while updating your Post
            </Alert>
          ) : (
            <div></div>
          )}
